...
class SupportRenderer implements IRenderer {
  ...
  constructor() {
    this.prototypes.set('fixed', new Konva.Path({
    ...
    this.prototypes.set('pinned', new Konva.Path({
    ...
    this.prototypes.set('roller', new Konva.Path({
    ...
    this.prototypes.set('verticalRoller', new Konva.Path({
  ...
  public draw(model: Model, viewport: Viewport, layer: Konva.Layer): void {
    ...
    model.nodes.forEach((node, index) => {
      ...
      if (constraint.fixedX && constraint.fixedY && constraint.fixedRotation) {
        supportType = 'fixed';
      } else if (constraint.fixedX && constraint.fixedY && !constraint.fixedRotation) {
        supportType = 'pinned';
      } else if (!constraint.fixedX && constraint.fixedY && !constraint.fixedRotation) {
        supportType = 'roller';
      } else if (constraint.fixedX && !constraint.fixedY && !constraint.fixedRotation) {
      ...
      if (supportType && this.prototypes.has(supportType)) {
        ...
        if (supportType === 'verticalRoller') {
          const hasLeftElement = model.elements.some(element => {
            ...
            if (otherNode && otherNode.dy === node.dy && otherNode.dx < node.dx) {
