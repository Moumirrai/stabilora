...
export class AddNodeCommand implements ICommand {
    ...
    do(): void {
        ...
        // store the newly created node object
    ...
    undo(): void {
        if (this.createdNode) {
            ...
            internalStore.update(model => {
                const remainingNodes = (model.nodes || []).filter(n => n.id !== nodeIdToRemove);
                const remainingElements = (model.elements || []).filter(
                    el => el.nodeA.id !== nodeIdToRemove && el.nodeB.id !== nodeIdToRemove
                );
                return {
                    ...model,
