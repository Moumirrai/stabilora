...
function clipLineSegment(line: [number, number, number, number], viewport: Konva.RectConfig): [number, number, number, number] | null {
    let [x1, y1, x2, y2] = line;
    const { x: xmin, y: ymin, width, height } = viewport;
    if (xmin === undefined || ymin === undefined || width === undefined || height === undefined) {
...
class ElementRenderer {
    ...
    public updateElement(element: Element): void {
        ...
        const elementGroup = this.targetLayer.findOne(`#element-${element.id}`) as Konva.Group;
        if (!elementGroup) {
     ...
     */
    private updateElementVisibilityAndClipping(element: Element, elementGroup: Konva.Group, scale: number): void {
        ...
        const bufferedViewport = {
            ...
            width: viewportRect.width + 2 * this.cullingBufferMultiplier * viewportRect.width,
            height: viewportRect.height + 2 * this.cullingBufferMultiplier * viewportRect.height
        };
        const isVisible = Konva.Util.haveIntersection(elementBounds, bufferedViewport);
        ...
        const clippedSolidPoints = clipLineSegment([x1, y1, x2, y2], bufferedViewport);
    ...
    public drawElement(element: Element): Konva.Group | null {
        ...
        const isInitiallyVisible = Konva.Util.haveIntersection(elementBounds, bufferedViewport);
